...
import { Bookmark, Para, Surah } from "@/types/type";
import {
    getSurahList
} from "@/utils/quranDb";
...
export function matchesSurahSearch(surah: Surah, query: string): boolean {
  ...
  if (/^\d+$/.test(rawQuery)) {
    return surah.number === Number(rawQuery);
  }

  return [surah.englishName, surah.englishNameTranslation, surah.arabicName]
...
export const useQuran = () => {
  ...
  const [selectedTab, setSelectedTab] = useState("All Surahs");
  ...
  const filterTabs = isLoggedIn ? ["All Surahs", "Bookmarks"] : ["All Surahs"];
  ...
  const checkDownloadStatus = async () => {
    try {
      ...
      setErrorMsg(
        err.message || "An error occurred while loading the Quran list.",
      );
    }
  };
  ...
  const loadSurahs = async () => {
    ...
    try {
      ...
      setErrorMsg(
        err.message || "An error occurred while fetching the Quran list.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const filteredSurahs = surahs.filter((surah) =>
    matchesSurahSearch(surah, deferredSearchQuery),
  );
